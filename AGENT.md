...
2.  **Documentation**:
    ...
    *   **API/Events**: If you change the data format broadcasted by the bot (structs in `types.rs` or `custom` JSON in strategies), you **MUST** update `docs/api/websocket_events.md` so the frontend team stays in sync.
